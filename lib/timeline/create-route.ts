...
export const TIMELINE_CREATE_MODE_QUERY_KEY = "createMode";
export const TIMELINE_CREATE_MODE_VALUES = [
  "ai",
  "aiMulti",
  "aiSingle",
  "chooser",
...
const TIMELINE_CREATE_PREFILL_KEYS = [
  ...
  "predecessorEventIds",
  "successorEventIds",
  TIMELINE_CREATE_MODE_QUERY_KEY,
] as const;
export type TimelineCreateMode = "aiMulti" | "aiSingle" | "chooser" | "manual";
...
type TimelineCreatePrefill = {
  createMode?: (typeof TIMELINE_CREATE_MODE_VALUES)[number];
...
export function buildTimelineCreateHref(prefill: TimelineCreatePrefill = {}) {
  ...
  if (prefill.createMode && TIMELINE_CREATE_MODE_VALUES.includes(prefill.createMode)) {
    params.set(TIMELINE_CREATE_MODE_QUERY_KEY, prefill.createMode);
  }
...
export function getTimelineCreateMode(
  searchParams: TimelineCreateSearchParams
): TimelineCreateMode {
  const value = readQueryValue(searchParams, TIMELINE_CREATE_MODE_QUERY_KEY);

  if (value === "manual" || value === "chooser") {
    return value;
  }
