...
import {
  ...
  createEmptyCultureFormValues,
  normalizeCultureFormValues,
  type CultureFormValues,
  type NormalizedCultureFormValues,
} from "@/types/culture";

type CultureFormProps = {
  initialValues?: CultureFormValues;
  submitLabel: string;
  onSubmit: (values: NormalizedCultureFormValues) => Promise<void>;
};
...
export function CultureForm({
  ...
  const [values, setValues] = useState<CultureFormValues>(() =>
    initialValues ?? createEmptyCultureFormValues()
  );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const normalizedValues = normalizeCultureFormValues(values);

    if (!normalizedValues.name) {
      setError("Culture name is required.");
      return;
    }
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : "Unable to save this culture.");
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <Field
          ...
          placeholder="loc_001, loc_river_district"
          hint="Comma separated"
        />
        <Field
          label="Language IDs"
          value={values.languageIds}
          onChange={(value) => updateField("languageIds", value)}
          placeholder="language_001"
          hint="Comma separated"
        />
      </section>
      ...
      <TextareaField
        ...
        placeholder="One-paragraph overview for culture list views."
        rows={3}
      />
      ...
      <TextareaField
        ...
        placeholder="Longer internal culture description for worldbuilding reference."
        rows={5}
      />
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <Field
          ...
          onChange={(value) => updateField("coreValues", value)}
          placeholder="Memory, Reciprocity, Practical honor"
          hint="Comma separated"
        />
        <Field
          label="Traditions"
          value={values.traditions}
          onChange={(value) => updateField("traditions", value)}
          placeholder="Lantern vigils, Winter ledger readings"
          hint="Comma separated"
        />
      </section>
  ...
  function updateField<Key extends keyof CultureFormValues>(
    key: Key,
    value: CultureFormValues[Key]
  ) {
    if (error) {
      setError(null);
    }
