...
- profile menu with a profile lightbox, AI-access toggles, API-key management, password-confirmed project deletion, password-confirmed full account deletion, and logout action
...
Books, Chapters, Scenes, Characters, Relationships, Factions, Cultures, Religions, Governments, Organizations, Plot Threads, Outlines, Glossary Terms, Eras, Themes, Languages, Species, Items, Technologies, Locations, Timeline Events, Notes, Retcons, Attachments, and AI Sessions currently have list, create, detail, and edit flows. `/timeline` remains the sole top-level chronology surface for browsing and creating timeline records. The workspace adds a visual center-line chronology, quick navigation, filters, derived insertion notches, compressed time-jump markers, inline create/edit entry points, and a toolbar brain-dump lightbox on top of `timeline_events`, while Timeline Events supports picker-style linking for the main connected slices. Linked entity detail pages now also support private Supabase Storage image uploads backed by scoped `attachments` rows. The profile lightbox now also includes an AI access tab with creative-versus-organizational capability toggles, a Security tab with password-confirmed project deletion that clears scoped records and uploaded files, plus full account deletion that removes the user's Supabase auth login, profile, projects, scoped data, and uploads before sending them back to sign-up. AI Sessions now also has a first-pass brain-dump flow plus a first-pass manuscript-import workflow that uses the authenticated user's saved OpenAI key, classifies brain dump under creative AI access and manuscript import under organizational AI access, greys those features out when their capability is off, and keeps all extracted proposals review-first on the scoped `ai_sessions` row before any canon write happens. Dedicated slice delete flows, richer cross-entity linking, deeper timeline continuity tooling, broader file workflows, automatic canon import without explicit review, and broader AI tooling are still future work.
...
- `SUPABASE_SERVICE_ROLE_KEY` for full account deletion from Supabase Auth
