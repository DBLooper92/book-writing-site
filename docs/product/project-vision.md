...
The Timeline workspace now exists as a real top-level chronology surface derived from `timeline_events`. Attachments now backs private image uploads on linked entity detail pages while the manual attachment form remains metadata-first for broader file references. AI Sessions now includes a staged brain-dump workflow that produces reviewable proposals, performs cheap matching and targeted context retrieval, and then lets the author explicitly apply approved timeline, character, chapter, and scene proposals into canon without auto-writing raw AI output. Broader non-image attachment workflows, deeper chronology tooling, broader-slice proposal expansion, and broader AI-writing operations are still future work.
