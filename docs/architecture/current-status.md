...
- the manuscript editor now measures the rendered page width for each chapter block before wrapping text and placing the custom caret, so embedded split-screen pages use the same line model as the visible page instead of full-width standalone wrapping
