...
- the manuscript editor now keeps the same manuscript page model in standalone and embedded split-screen modes, scaling the full page down in narrow panes instead of changing line breaks or page counts; the trailing `Start a New Chapter` action page is only shown after the current last chapter and is excluded from running headers and manuscript page numbering
- the manuscript editor pen-name menu can create a new profile pen name in a lightbox and immediately assign it to the selected book; the first saved pen name also becomes the profile default through app settings
