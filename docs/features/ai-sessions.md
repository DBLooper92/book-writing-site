...
- creative AI access now gates brain-dump entry points plus brain-dump review, targeted-context, and apply controls
- organizational AI access now gates manuscript-import entry points plus mapping, processing, review, and apply controls
- disabled AI capabilities stay visible in the UI as greyed, non-interactive controls, and the matching routes now reject direct requests while the capability is off
...
- manuscript import now also has a dedicated left-rail entry labeled `Upload Book/ Series`, which links straight to `/ai-sessions/manuscript-import` and greys out when organizational AI access is turned off
- the AI Sessions workspace keeps the metadata-only create flow available even when one or both AI capability toggles are off, because that route does not itself call the provider or run AI workflow actions
