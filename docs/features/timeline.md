...
- the main chronology toolbar can launch a brain-dump lightbox beside the create button, but that flow should still use the signed-in user's saved OpenAI key, respect the profile's creative-AI access toggle, and save its extraction output onto project-scoped `ai_sessions` rows rather than directly creating canon records
