...
type CharacterProposalReviewUpdateInput = {
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    payload = normalizeCharacterProposalReviewUpdateInput(await request.json());
  } catch (error) {
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Invalid character proposal review update.",
      },
      { status: 400 }
    );
  }
  ...
  const proposal = extractionResult.characters[payload.proposalIndex];
  ...
  if (payload.matchedRecordId && !matchedRecord) {
    return NextResponse.json({ error: "Selected character match is no longer available." }, { status: 400 });
  }
  ...
  const updatedExtractionResult = {
    ...
    characters: extractionResult.characters.map((characterProposal, index) =>
      index === payload.proposalIndex ? updatedProposal : characterProposal
    ),
  };
...
function normalizeCharacterProposalReviewUpdateInput(
  value: unknown
): CharacterProposalReviewUpdateInput {
  if (!isRecord(value)) {
