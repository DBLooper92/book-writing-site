...
type ChapterProposalReviewUpdateInput = {
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  let payload: ChapterProposalReviewUpdateInput;

  try {
    payload = normalizeChapterProposalReviewUpdateInput(await request.json());
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Invalid chapter proposal review update.",
      },
      { status: 400 }
    );
  }
  ...
  const proposal = extractionResult.chapterOutlines[payload.proposalIndex];
  ...
  if (payload.matchedRecordId && !matchedRecord) {
    return NextResponse.json({ error: "Selected chapter match is no longer available." }, { status: 400 });
  }
  ...
  const updatedExtractionResult = {
    ...
    chapterOutlines: extractionResult.chapterOutlines.map((chapterProposal, index) =>
      index === payload.proposalIndex ? updatedProposal : chapterProposal
    ),
  };
...
function normalizeChapterProposalReviewUpdateInput(
  value: unknown
): ChapterProposalReviewUpdateInput {
  if (!isRecord(value)) {
