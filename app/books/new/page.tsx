...
export default function NewBookPage() {
  ...
  async function handleCreateBook(values: NormalizedBookFormValues) {
    if (!uid || !activeProjectId) {
      throw new Error("Select an active project before creating a book.");
    }
  ...
  return (
    ...
    >
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-600">
              {activeProject
                ...
                : "A project must be active before book creation can continue."}
            </p>
          </div>
      ...
      {!user ? (
        ...
        <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
          <BookForm submitLabel="Create book" onSubmit={handleCreateBook} />
        </section>
      )}
    </PageShell>
  );
