...
export default function NewPlotThreadPage() {
  const router = useRouter();
  const { user, uid, activeProjectId, activeProject, loading } = useActiveProject();

  async function handleCreatePlotThread(values: NormalizedPlotThreadFormValues) {
    if (!uid || !activeProjectId) {
      throw new Error("Select an active project before creating a plot thread.");
    }
    ...
    >
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-600">
              {activeProject
                ...
                : "A project must be active before plot-thread creation can continue."}
            </p>
          </div>
      ...
      {!user ? (
        <StateCard tone="warning">
          Sign in first to create plot threads.{" "}
        ...
        <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
          <PlotThreadForm
            ...
            onSubmit={handleCreatePlotThread}
          />
        </section>
      )}
    </PageShell>
