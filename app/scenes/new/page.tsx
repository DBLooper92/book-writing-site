...
export default function NewScenePage() {
  ...
  async function handleCreateScene(values: NormalizedSceneFormValues) {
    if (!uid || !activeProjectId) {
      throw new Error("Select an active project before creating a scene.");
    }
    ...
    >
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-600">
              {activeProject
                ...
                : "A project must be active before scene creation can continue."}
            </p>
          </div>
      ...
      {!user ? (
        <StateCard tone="warning">
          Sign in first to create scenes.{" "}
        ...
        <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
          <SceneForm submitLabel="Create scene" onSubmit={handleCreateScene} />
        </section>
      )}
    </PageShell>
  );
